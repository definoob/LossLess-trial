...
describe('LERC20', () => {
  ...
  describe('transfer', () => {
    describe('when isLossLess is on', () => {
      it('should revert when you send token to blacklisted address', async () => {
        await erc20.connect(admin).whitelistAddress(initialHolder.address);
        await erc20.connect(admin).blacklistAddress(anotherAccount.address);
        await expect(
          erc20.connect(initialHolder).transfer(anotherAccount.address, 50),
        ).to.be.revertedWith('Recipient is blacklisted');
      });

      it('should revert when you send token from blacklisted address', async () => {
        await erc20.connect(admin).blacklistAddress(initialHolder.address);
        await erc20.connect(admin).whitelistAddress(anotherAccount.address);
        await expect(
          erc20.connect(initialHolder).transfer(anotherAccount.address, 50),
    ...
    it('should revert when balance is not enough', async () => {
      await expect(
        erc20.connect(initialHolder).transfer(anotherAccount.address, 1000),
