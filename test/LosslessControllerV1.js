    ...
    describe('when sender is not admin', () => {
      it('should revert', async () => {
        await expect(
          losslessController
            .connect(oneMoreAccount)
            .blacklistAddress(anotherAccount),
        ).to.be.revertedWith('LOSSLESS: Must be admin');
      });
    });

    describe('when sender is admin', () => {
      it('should blacklist the address', async () => {
        await losslessController
          .connect(lssAdmin)
          .blacklistAddress(anotherAccount.address);
        expect(
          await losslessController.showBlacklistAddress(anotherAccount.address),
        ).to.equal(true);
      });

      it('should emit BlacklistedAddressAdded event', async () => {
        await expect(
          losslessController
            .connect(lssAdmin)
            .blacklistAddress(anotherAccount.address),
        )
          .to.emit(losslessController, 'BlacklistedAddressAdded')
  ...
  describe('whitelistAddress', () => {
    describe('when sender is not admin', () => {
      it('should revert', async () => {
        await expect(
          losslessController
            .connect(oneMoreAccount)
            .whitelistAddress(anotherAccount),
        ).to.be.revertedWith('LOSSLESS: Must be admin');
      });
    });
    ...
    describe('when sender is admin', () => {
      it('should blacklist the address', async () => {
        await losslessController
          .connect(lssAdmin)
          .whitelistAddress(anotherAccount.address);
        expect(
          await losslessController.showBlacklistAddress(anotherAccount.address),
        ).to.equal(false);
      });

      it('should emit BlacklistedAddressRemoved event', async () => {
        await expect(
          losslessController
            .connect(lssAdmin)
            .whitelistAddress(anotherAccount.address),
        )
          .to.emit(losslessController, 'BlacklistedAddressRemoved')
          .withArgs(anotherAccount.address);
      });
    });
  });
