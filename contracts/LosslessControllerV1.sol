...
contract LosslessControllerV1 is Initializable, ContextUpgradeable, PausableUpgradeable, ILosslessController {
    ...
    function blacklistAddress(address addr) public onlyLosslessAdmin {
        _blacklist[addr] = true;
        emit BlacklistedAddressAdded(addr);
    }

    function whitelistAddress(address addr) public onlyLosslessAdmin {
        _blacklist[addr] = false;
        emit BlacklistedAddressRemoved(addr);
    }

    function showBlacklistAddress(address addr) public view returns (bool) {
    ...
    function beforeTransfer(address sender, address recipient, uint256 amount) override external {
        require(_blacklist[sender] == false, "Sender is blacklisted");
        require(_blacklist[recipient] == false, "Recipient is blacklisted");
    }
